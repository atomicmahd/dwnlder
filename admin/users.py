...
def register(app: Client):
    ...
    async def add_user_cmd(client: Client, message: Message):
        ...
        try:
            ...
            await message.reply(
                f"✅ Added: **{user.first_name}** (`{user.id}`)\n"
                f"Username: @{user.username or 'N/A'}",
                parse_mode="markdown",
            )
        except Exception as e:
            await message.reply(f"❌ Failed: `{e}`", parse_mode="markdown")
    ...
    async def remove_user_cmd(client: Client, message: Message):
        ...
        try:
            ...
            # notify the user
            try:
                await client.send_message(user_id, "❌ Your access to this bot has been removed.")
            except Exception:
                pass

            await message.reply(f"✅ User `{user_id}` removed.", parse_mode="markdown")
        except ValueError:
    ...
    async def ban_user_cmd(client: Client, message: Message):
        ...
        try:
            ...
            await message.reply(f"✅ User `{user_id}` banned.", parse_mode="markdown")
        except ValueError:
            await message.reply("❌ Invalid user ID.")
    ...
    async def unban_user_cmd(client: Client, message: Message):
        ...
        try:
            ...
            await message.reply(f"✅ User `{user_id}` unbanned.", parse_mode="markdown")
        except ValueError:
            await message.reply("❌ Invalid user ID.")
    ...
    async def set_limit_cmd(client: Client, message: Message):
        ...
        try:
            ...
            await message.reply(
                f"✅ Limits updated for `{user_id}`:\n"
                f"  Max file: `{max_file or 'default'} MB`\n"
                f"  Daily: `{daily or 'default'} MB`\n"
                f"  Queue: `{queue or 'default'}`",
                parse_mode="markdown",
            )
        except ValueError:
            await message.reply("❌ All values must be integers.")
