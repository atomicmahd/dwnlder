...
async def upload_file(
    ...
    if not job.file_path or not os.path.exists(job.file_path):
        await _edit(status_message, "❌ File not found after download.")
        return False

    for attempt in range(1, config.UPLOAD_MAX_RETRIES + 1):
        try:
            await _edit(status_message, f"📤 Uploading... (attempt {attempt}/{config.UPLOAD_MAX_RETRIES})")
...
async def _send_file(client: Client, job: DownloadJob, status_message: Message):
    path = job.file_path
    chat_id = job.chat_id
    caption = f"✅ Downloaded via @{(await client.get_me()).username}"
...
async def retry_cached_upload(client: Client, job_id: str, telegram_id: int, status_message: Message) -> bool:
    ...
    if not cached:
        await _edit(status_message, "❌ Cache expired or not found. Please re-send the link.")
        return False
    ...
    if not os.path.exists(job.file_path):
        await _edit(status_message, "❌ Cached file no longer exists. Please re-send the link.")
