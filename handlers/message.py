...
def register(app: Client):
    ...
    async def handle_text(client: Client, message: Message):
        ...
        if not urls:
            await message.reply("Send me a link and I'll download it.")
            return
        ...
        if not handler:
            await message.reply("❌ Unsupported link. Try a YouTube, Instagram, TikTok, X, or direct media URL.")
            return
        ...
        await message.reply(
            f"🔗 **{handler.PLATFORM_NAME.title()}** link detected.\nSelect quality:",
    ...
    async def handle_retry_command(client: Client, message: Message):
        ...
        status_msg = await message.reply("🔄 Retrying upload...")
...
async def dispatch_download(client: Client, job: DownloadJob, quality: str, status_msg):
    ...
    if not handler:
        await status_msg.edit_text("❌ Handler not found.")
