...
export default function Links() {
  ...
  return (
    <div style={S.page}>
      <div style={S.title}>🔗 Link History ({links.length})</div>
      <div style={S.filters}>
        <input style={S.input} placeholder="Search URL, title, user..." value={search} onChange={e => setSearch(e.target.value)} />
      ...
      <table style={S.table}>
        <thead>
          <tr>{['Time','User','Platform','Title','Quality','Size','Status'].map(h => <th key={h} style={S.th}>{h}</th>)}</tr>
        </thead>
        <tbody>
          {links.map(l => (
            <tr key={l.id}>
              <td style={S.td}><span style={{ color: '#64748b', fontSize: 11 }}>{new Date(l.time).toLocaleString()}</span></td>
              <td style={S.td}>
