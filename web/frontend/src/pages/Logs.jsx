...
export default function Logs() {
  ...
  const clearLogs = async () => { if (confirm('Clear all logs?')) { await api.clearLogs(); setLogs([]) } }
  ...
  return (
    <div style={S.page}>
      <div style={S.header}>
        <div style={S.title}>
          📋 Logs
          {live && <span style={{ marginLeft: 12, fontSize: 13, fontWeight: 400 }}><span style={S.dot(wsOk)} />{wsOk ? 'Live' : 'Connecting...'}</span>}
        </div>
        <div style={S.controls}>
          ...
          <button style={S.liveBtn(live)} onClick={() => setLive(p => !p)}>{live ? '🔴 Live' : '⚫ Live'}</button>
