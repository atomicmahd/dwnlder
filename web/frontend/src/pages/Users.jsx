...
const S = {
  ...
  th: { padding: '12px 16px', textAlign: 'left', fontSize: 12, color: '#64748b', fontWeight: 600, borderBottom: '1px solid #1e2a45', textTransform: 'uppercase' },
  ...
  actionBtn: (color) => ({ padding: '5px 12px', borderRadius: 6, border: 'none', cursor: 'pointer', fontSize: 12, fontWeight: 600, background: color, color: '#fff', marginRight: 4 }),
...
export default function Users() {
  ...
  const removeUser = async (u) => { if (confirm(`Remove ${u.display_name}?`)) { await api.removeUser(u.telegram_id); load() } }
  const banUser = async () => { await api.banUser(target.telegram_id, form.msg || 'You have been suspended.'); load(); closeModal() }
  ...
  return (
    <div style={S.page}>
      <div style={S.header}>
        ...
        <button style={S.addBtn} onClick={() => openModal('add')}>+ Add User</button>
      </div>

      <table style={S.table}>
        <thead>
          <tr>{['User', 'ID', 'Status', 'Last Seen', 'Actions'].map(h => <th key={h} style={S.th}>{h}</th>)}</tr>
        </thead>
        <tbody>
          {users.map(u => (
            <tr key={u.telegram_id}>
              <td style={S.td}>
                ...
                <div style={{ color: '#64748b', fontSize: 12 }}>@{u.username || 'N/A'}</div>
              </td>
              <td style={S.td}><code style={{ color: '#94a3b8', fontSize: 12 }}>{u.telegram_id}</code></td>
              <td style={S.td}><span style={S.badge(u.is_active, u.is_banned)}>{u.is_banned ? 'Banned' : u.is_active ? 'Active' : 'Inactive'}</span></td>
              <td style={S.td}><span style={{ color: '#64748b', fontSize: 12 }}>{u.last_seen ? new Date(u.last_seen).toLocaleDateString() : 'Never'}</span></td>
              <td style={S.td}>
                <button style={S.actionBtn('#1d4ed8')} onClick={() => openModal('limits', u)}>Limits</button>
                {u.is_banned
                  ? <button style={S.actionBtn('#15803d')} onClick={() => unbanUser(u)}>Unban</button>
                  : <button style={S.actionBtn('#b45309')} onClick={() => openModal('ban', u)}>Ban</button>
                }
                <button style={S.actionBtn('#7f1d1d')} onClick={() => removeUser(u)}>Remove</button>
              </td>
            </tr>
          ))}
        </tbody>
      ...
      {modal === 'add' && (
        <div style={S.modal}>
          <div style={S.modalCard}>
            <div style={S.modalTitle}>Add User</div>
            <label style={S.label}>Telegram ID</label>
            <input style={S.input} placeholder="123456789" onChange={f('id')} />
            <label style={S.label}>Username (optional)</label>
            <input style={S.input} placeholder="@username" onChange={f('username')} />
            <label style={S.label}>Display Name (optional)</label>
            <input style={S.input} onChange={f('name')} />
            <div style={S.row}>
              <button style={S.confirmBtn} onClick={addUser}>Add</button>
      ...
      {modal === 'ban' && (
        <div style={S.modal}>
          <div style={S.modalCard}>
            <div style={S.modalTitle}>Ban {target?.display_name}</div>
            <label style={S.label}>Message to user</label>
            <input style={S.input} placeholder="Your access has been suspended." onChange={f('msg')} />
            <div style={S.row}>
              <button style={{ ...S.confirmBtn, background: '#dc2626' }} onClick={banUser}>Ban</button>
              <button style={S.cancelBtn} onClick={closeModal}>Cancel</button>
            </div>
          </div>
        </div>
      ...
      {modal === 'limits' && (
        <div style={S.modal}>
          <div style={S.modalCard}>
            <div style={S.modalTitle}>Limits for {target?.display_name}</div>
            <label style={S.label}>Max file size (MB) — leave blank for default</label>
            <input style={S.input} type="number" defaultValue={target?.max_file_mb || ''} onChange={f('file')} />
            <label style={S.label}>Daily limit (MB) — leave blank for default</label>
            <input style={S.input} type="number" defaultValue={target?.daily_limit_mb || ''} onChange={f('daily')} />
            <label style={S.label}>Queue limit — leave blank for default</label>
            <input style={S.input} type="number" defaultValue={target?.queue_limit || ''} onChange={f('queue')} />
            <div style={S.row}>
              <button style={S.confirmBtn} onClick={setLimits}>Save</button>
              <button style={S.cancelBtn} onClick={closeModal}>Cancel</button>
            </div>
          </div>
        </div>
